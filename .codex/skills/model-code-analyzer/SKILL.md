...
- capability IDs from `planning/capability_checklist.json`, when present;
...
- `validate_capability()` assertions translated from each `falsifiable_check`;
...
The plan must make degradation impossible to hide:

- Implement exactly the approved method; a replacement paradigm that is "easier to code" is a scope violation.
- No proxy capabilities: labels from file IDs, filenames, or row indices; same-data derived features predicting derived labels; string/word matching standing in for semantic capabilities; scoring without performing the claimed downstream action.
- Every capability with a `falsifiable_check` gets a runtime assertion that raises on failure rather than a comment or a warning.
- Result files are saved at full precision; the agent reads only summaries. Audit scripts print conclusions and at most five violation locations, never whole arrays.
...
- Do not plan a proxy implementation for a claimed capability; degrade the claim through the human decision path instead.
...
- Capability assertions and data-summary rules are explicit in the plan when the checklist exists.
