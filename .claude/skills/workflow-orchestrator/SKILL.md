...
- parse/classification present and problem is parameter-dense or has multiple verifiable capabilities → `capability-contract-builder`;
...
- `planning/capability_checklist.json` is optional and does not change G1 pass conditions. When present, treat it as the cross-stage acceptance contract: modeling claims capability IDs, code plans and reviews verify `falsifiable_check`, and the paper may claim only passed capabilities.
- `paper/audits/output_spec_report.md` is a submission-profile pre-flight before the three G6 auditors. It does not replace any of them, and a `FAILED` verdict blocks final assembly until repaired.
