...
3. Evaluate:
   ...
   - `reproducibility`
   - `output_contract`
4. When `planning/capability_checklist.json` exists, add the `capability_contract` check: every capability's `falsifiable_check` has a runtime assertion or equivalent executable evidence; no proxy labels or string matching standing in for semantic capabilities.
5. Include compatibility evidence for toolbox usage, `jsonencode`, file I/O, plotting/export, and 北太天元 constraints.
6. Add only relevant numerical, feasibility, leakage, or scale checks.
7. If runtime is unavailable, use `NOT_RUN` rather than claiming execution success.
8. If asked to fix findings, patch minimally and rerun affected checks.
9. Save `code/matlab/Qx/reviews/qx_matlab_review.json`.
...
The optional `capability_contract` check is `NOT_APPLICABLE` when no checklist exists.
...
- Do not weaken or delete a runtime capability assertion to make the check pass.
